...
exports.updateUser = async (req, res) => {
    ...
    try {
        ...
        if (typeof isApproved === 'boolean' && user.role === 'organizer') {
...
exports.getAllBookings = async (req, res) => {
    try {
        ...
        if (status && ['PaymentPending', 'Confirmed', 'Cancelled', 'CheckedIn', 'PaymentFailed'].includes(status)) query.status = status;
        if (bookingRefId) query.bookingRefId = bookingRefId.trim().toUpperCase();
        ...
        if (date) {
            ...
            } catch (e) { console.warn("Invalid date format for booking filter:", date); }
        }

        let sortOptions = { bookingTime: -1 };
        if (sort) { /* ... your sort logic ... */ }
        ...
        const bookings = await Booking.find(query)
            ...
            .populate({
                ...
                populate: [ 
                    { path: 'venue', model: 'Venue', select: 'name' }
                ]
            })
        ...
        for (let booking of bookings) {
            if (booking.showtime) {
                if (booking.showtime.movie) {
                    ...
                    if (!booking.showtime.movie.title) { 
                         const movieDoc = await Movie.findById(booking.showtime.movie).select('title').lean();
                         if (movieDoc) booking.showtime.movie = movieDoc; 
                    }
                }
                if (booking.showtime.event) {
                    if (!booking.showtime.event.title) { 
                        const eventDoc = await Event.findById(booking.showtime.event).select('title').lean();
        ...
        res.status(500).json({ msg: 'Server error fetching bookings', error: err.message, path: err.path });
    }
};
...
exports.getBookingByIdAdmin = async (req, res) => {
    ...
    try {
            ...
            .populate({
                ...
                populate: [ 
                    ...
                    
                    {
                        ...
                        populate: { path: 'organizer', model: 'User', select: 'name organizationName email' } 
                    }
                ]
            })
        ...
        if (booking.showtime) {
            if (booking.showtime.movie && typeof booking.showtime.movie === 'object' && !booking.showtime.movie.title) { 
            ...
            if (booking.showtime.event && typeof booking.showtime.event === 'object' && !booking.showtime.event.title) { 
...
exports.deleteCity = async (req, res) => {
    ...
    try {
        ...
        await city.remove();
        res.status(200).json({ success: true, msg: 'City deleted' });
