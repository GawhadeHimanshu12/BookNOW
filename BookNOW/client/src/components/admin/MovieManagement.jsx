...
const commonGenres = ['Action', 'Comedy', 'Drama', 'Sci-Fi', 'Thriller', 'Horror', 'Romance', 'Animation', 'Family', 'Fantasy', 'Adventure', 'Crime', 'Mystery', 'Historical', 'Biographical'];
...
const MovieManagement = () => {
    ...
    const handleOpenModal = (movie = null) => {
        ...
        if (movie) {
            ...
            setFormatSelection(Array.isArray(movie.format) ? movie.format : []);
        } else {
            setIsEditing(false);
    ...
    const handleFormatSelectChange = (event) => {
        ...
        setFormatSelection(newSelectedFormats);
    ...
    return (
        <Box sx={{p:1}}>
            ...
            {movies.length === 0 && !isLoading && <Typography sx={{p:2, textAlign: 'center'}}>No movies found.</Typography>}
                    ...
                    <React.Fragment key={movie._id}>
                        ...
                        >
                            <ListItemText
                                ...
                                secondary={
                                    <Box>
                                        Language: {movie.movieLanguage || movie.language} | Duration: {movie.duration} min
                                        <Typography variant="caption" display="block">Release: {dayjs(movie.releaseDate).format('DD MMM YYYY')}</Typography>
                                        <Typography variant="caption" display="block">Genres: {(movie.genre || []).join(', ') || 'N/A'}</Typography>
                ...
                <Box component="form" onSubmit={handleSubmit}>
                    <DialogContent>
                        ...
                        <Grid container spacing={2}>
                            {/* ... other Grid items for title, language, description, releaseDate, duration, censorRating ... */}
                            <Grid item xs={12} sm={8}>
                                <TextField name="title" label="Title" value={currentMovie.title} onChange={handleChange} fullWidth required margin="dense"/>
                            </Grid>
                            <Grid item xs={12} sm={4}>
                                <TextField name="movieLanguage" label="Language" value={currentMovie.movieLanguage} onChange={handleChange} fullWidth required margin="dense"/>
                            </Grid>
                            <Grid item xs={12}>
                                <TextField name="description" label="Description" value={currentMovie.description} onChange={handleChange} fullWidth required multiline rows={3} margin="dense"/>
                            </Grid>
                            <Grid item xs={12} sm={4}>
                                <DatePicker label="Release Date" value={currentMovie.releaseDate} onChange={handleDateChange} slotProps={{ textField: { fullWidth: true, required: true, margin:"dense" } }} />
                            </Grid>
                            <Grid item xs={12} sm={4}>
                                <TextField name="duration" label="Duration (minutes)" type="number" value={currentMovie.duration} onChange={handleChange} fullWidth required inputProps={{min: 0}} margin="dense"/>
                            </Grid>
                            <Grid item xs={12} sm={4}>
                                <FormControl fullWidth margin="dense">
                                    <InputLabel id="censor-rating-label">Censor Rating</InputLabel>
                                    <Select labelId="censor-rating-label" name="censorRating" value={currentMovie.censorRating} label="Censor Rating" onChange={handleChange}>
                            ...
                            <Grid item xs={12} sm={9}>
                                <TextField value={genreInput} onChange={handleArrayInputChange(setGenreInput)} label="Add Genre" fullWidth margin="dense" size="small" helperText="Type and press Add"/>
                            </Grid>
                            <Grid item xs={12} sm={3} sx={{display:'flex', alignItems:'center'}}>
                                <Button onClick={() => addToArrayField('genre', genreInput, setGenreInput)} variant="outlined" size="small" fullWidth sx={{height: '40px', mt: '8px'}}>Add Genre</Button>
                            </Grid>
                            <Grid item xs={12}>
                                {(currentMovie.genre || []).map(g => <Chip key={g} label={g} onDelete={() => removeFromArrayField('genre', g)} sx={{mr:0.5, mb:0.5}}/>)}
                            </Grid>

                            {/* Cast Section */}
                            <Grid item xs={12}> <Typography variant="subtitle2" sx={{mt:1}}>Cast</Typography> </Grid>
                             <Grid item xs={12} sm={9}>
                                <TextField value={castInput} onChange={handleArrayInputChange(setCastInput)} label="Add Cast Member" fullWidth margin="dense" size="small"/>
                            </Grid>
                            <Grid item xs={12} sm={3} sx={{display:'flex', alignItems:'center'}}>
                                <Button onClick={() => addToArrayField('cast', castInput, setCastInput)} variant="outlined" size="small" fullWidth sx={{height: '40px', mt: '8px'}}>Add Cast</Button>
                            </Grid>
                            <Grid item xs={12}>
                                {(currentMovie.cast || []).map(c => <Chip key={c} label={c} onDelete={() => removeFromArrayField('cast', c)} sx={{mr:0.5, mb:0.5}}/>)}
                            </Grid>
                            
                            {/* Crew Section */}
                            <Grid item xs={12}> <Typography variant="subtitle2" sx={{mt:1}}>Crew</Typography> </Grid>
                             <Grid item xs={12} sm={9}>
                                <TextField value={crewInput} onChange={handleArrayInputChange(setCrewInput)} label="Add Crew Member (e.g., Director: Name)" fullWidth margin="dense" size="small"/>
                            </Grid>
                            <Grid item xs={12} sm={3} sx={{display:'flex', alignItems:'center'}}>
                                <Button onClick={() => addToArrayField('crew', crewInput, setCrewInput)} variant="outlined" size="small" fullWidth sx={{height: '40px', mt: '8px'}}>Add Crew</Button>
                            </Grid>
                            <Grid item xs={12}>
                                {(currentMovie.crew || []).map(cr => <Chip key={cr} label={cr} onDelete={() => removeFromArrayField('crew', cr)} sx={{mr:0.5, mb:0.5}}/>)}
                            </Grid>

                            {/* Formats Section - Corrected */}
                            <Grid item xs={12}> <Typography variant="subtitle2" sx={{mt:1}}>Formats</Typography> </Grid>
                            <Grid item xs={12}>
                                <FormControl fullWidth margin="dense" size="small">
                                    <InputLabel id="formats-select-label">Select Formats</InputLabel>
                                    <Select
                                        ...
                                        onChange={(event) => {
                                            ...
                                            setCurrentMovie(prev => ({
                                                ...prev,
                                                format: typeof value === 'string' ? value.split(',') : value,
                                            }));
                                        }}
                            ...
                            {/* The "Add Format(s)" button is removed as selection is handled by the multi-select directly */}
                            <Grid item xs={12}>
                                {(currentMovie.format || []).map(f => <Chip key={f} label={f} onDelete={() => removeFromArrayField('format', f)} sx={{mr:0.5, mb:0.5}}/>)}
                            </Grid>
