...
exports.getReviewsForMovie = async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(movieId)) return res.status(400).json({ msg: 'Invalid Movie ID format' });
...
exports.createReview = async (req, res) => {
    ...
    try {
        ...
        if (existingReview) return res.status(400).json({ msg: 'You have already submitted a review for this movie' });

        const showtimeIds = await Showtime.find({ movie: movieId }).distinct('_id');
        if (showtimeIds.length === 0) return res.status(400).json({ msg: 'No showtimes found for this movie to verify booking.' });
        ...
        if (err.code === 11000) return res.status(400).json({ msg: 'You have already submitted a review for this movie (duplicate key).' });
        res.status(500).json({ msg: 'Server error' });
    }
};
...
exports.updateReview = async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(reviewId)) return res.status(400).json({ msg: 'Invalid Review ID format' });
    ...
    try {
        ...
        if (review.user.toString() !== userId) return res.status(403).json({ msg: 'User not authorized to update this review' });
...
exports.likeReview = async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(reviewId)) return res.status(400).json({ msg: 'Invalid Review ID' });
...
exports.reportReview = async (req, res) => {
    ...
    try {
        ...
        if (hasReported) return res.status(400).json({ msg: 'You have already reported this review.' });
